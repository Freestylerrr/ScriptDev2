...
bool GossipHello_npc_dragon_guide(Player* pPlayer, Creature* pCreature)
{
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,"Please, ressurect me so I can continue my fight!",GOSSIP_SENDER_MAIN,GOSSIP_ACTION_INFO_DEF+1);

    pPlayer->SEND_GOSSIP_MENU(pPlayer->GetGossipTextId(pCreature), pCreature->GetGUID());
    return true;
}

bool GossipSelect_npc_dragon_guide(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF+1)
    {
        pPlayer->CLOSE_GOSSIP_MENU();
        pPlayer->CastSpell(pPlayer,21074,true);               
    }
	return true;
}
...
void AddSC_azshara()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_dragon_guide";
    newscript->pGossipHello = &GossipHello_npc_dragon_guide;
    newscript->pGossipSelect = &GossipSelect_npc_dragon_guide;
    newscript->RegisterSelf();
}
