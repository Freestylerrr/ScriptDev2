...
bool GossipHello_npc_mohawk(Player* pPlayer, Creature* pCreature)
{
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,"I believe in you, Mr. T!",GOSSIP_SENDER_MAIN,GOSSIP_ACTION_INFO_DEF+1);

    pPlayer->SEND_GOSSIP_MENU(pPlayer->GetGossipTextId(0,0), pCreature->GetGUID());
    return true;
}

bool GossipSelect_npc_mohawk(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF+1)
    {
        pPlayer->CLOSE_GOSSIP_MENU();
        pPlayer->CastSpell(pPlayer,69243,true);               
    }
	return true;
}
...
void AddSC_npcs_special()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_npc_mohawk";
    newscript->pGossipHello = &GossipHello_npc_mohawk;
    newscript->pGossipSelect = &GossipSelect_npc_mohawk;
    newscript->RegisterSelf();
}
