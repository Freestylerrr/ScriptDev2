...
SDName: Netherstorm
SD%Complete: 80
SDComment: Quest support: 10438, 10652 (special flight paths), 10299, 10321, 10322, 10323, 10329, 10330, 10338, 10365(Shutting Down Manaforge), 10198, 10191
    ...
    void WaypointReached(uint32 uiWP)
    {
        switch (uiWP)
        {
            ...
            case 29:
                ...
                if ( GameObject* pGO = GetClosestGameObjectWithEntry(m_creature, GO_DRAENEI_MACHINE, INTERACTION_DISTANCE) )
                {
                    // take the GO -> animation
                    m_creature->HandleEmoteCommand(EMOTE_STATE_LOOT);
